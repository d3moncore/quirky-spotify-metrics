...
const TopArtists = () => {
  ...
  const { 
    items: artists, 
    loading 
  } = useTopItems<SpotifyArtist>('artists', timeRange);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="animate-pulse text-spotify-green font-semibold">Loading your top artists...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="pt-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <header className="mb-12 animate-in">
          <h1 className="text-3xl font-bold text-spotify-gradient mb-2">Your Top Artists</h1>
          <p className="text-muted-foreground">The artists that define your musical taste</p>
        </header>
        
        <Tabs defaultValue="medium_term" className="animate-in stagger-1" onValueChange={setTimeRange}>
        ...
        >
          <div className="flex flex-col h-full">
            ...
            <div className="flex justify-between text-sm text-muted-foreground mb-4">
              <div>
                <div className="font-medium text-foreground mb-1">Followers</div>
                <div>{formatNumber(artist.followers?.total || 0)}</div>
              </div>
              <div>
                <div className="font-medium text-foreground mb-1">Popularity</div>
                <div>{artist.popularity}%</div>
              </div>
            </div>
            
            <div className="mt-auto">
              <div className="text-sm font-medium text-foreground mb-2">Top Genres</div>
